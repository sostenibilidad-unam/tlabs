...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for e in EgoEdge.objects.filter(source__name__startswith="TL0"):
            i = EgoEdge.objects.get(source=e.target,
                                    target=e.source,
                                    phase=phase)
            e.influence_target = i.influence_source
            e.save()

            i.influence_source = 0
            i.save()
